...
import { applyAccountPlanPolicy, extractPlanType } from "./account-policy.ts";
import { ensureFreshAccount } from "./auth.ts";
...
export function startUsagePolling(store: Store, settings: Settings, encryptionKey: Buffer, logger: Logger): UsagePollingHandle {
  ...
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      const now = Date.now();
      const active = accounts.filter((account) => account.status === "active" && (states.get(account.id)?.nextAttemptAt ?? 0) <= now);
      const batches = chunkAccounts(active, concurrencyLimit);
      for (const batch of batches) {
        if (stopped) {
          break;
        }
        await Promise.all(batch.map((account) => refreshUsageWithBackoff(account, store, encryptionKey, settings, logger, states, jitterMs)));
      }
    } finally {
      running = false;
    }
...
// 3-2. Usage refresh with backoff
async function refreshUsageWithBackoff(
  ...
  try {
    ...
    const current = states.get(account.id) ?? {failureCount: 0, nextAttemptAt: 0};
