...
// 1. Account factory ――――――――――――――――――――――――――――――――――――――――――――――――――――――
function account(id: string, usedPercent: number, lastSelectedAt: number | null=null): Account {
  return {
    id,
    email: null,
...
describe("balancer", () => {
  ...
  it("prefers gpt-5.5 ready accounts ahead of fallback-only accounts", () => {
    const preferredReady = {
      ...account("a", 20),
      supportedModelIds: [PREFERRED_HIGH_CAPABILITY_MODEL, FALLBACK_HIGH_CAPABILITY_MODEL],
    };
    const fallbackOnly = {
      ...account("b", 5),
      supportedModelIds: [FALLBACK_HIGH_CAPABILITY_MODEL],
    };
    const unknown = account("c", 10);
    const ranked = rankAccounts([fallbackOnly, unknown, preferredReady], 101, {
...
describe("detectSharedResetEpoch", () => {
  ...
  it("returns the shared epoch when all values agree within tolerance", () => {
    assert.equal(detectSharedResetEpoch([1_777_057_806, 1_777_057_806, 1_777_057_806]), 1_777_057_806);
    assert.equal(detectSharedResetEpoch([1_777_057_806, 1_777_057_807], 2), 1_777_057_807);
  });
